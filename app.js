...
async function loadProgrammer(silent = false) {
    ...
    try {
        ...
        // -----------------------------------------------------------------
        // Her kalder vi MCR-vagterne og fodrer dem direkte til din nye popup
        // -----------------------------------------------------------------
        renderMcrVagter(jsonData.mcrVagter);
...
async function aabenTechModal(event, rowId) {
    ...
    document.getElementById('TechBackupPass').value = program["BACKUP SRT Passphrase"] || '';
...
async function gemProgram() {
    ...
    let emailSubject = "Produktionsdatabasen er opdateret"; 
...
function lukMcrModal() {
    document.getElementById('mcrModal').style.display = "none";
}

function renderMcrVagter(vagter) {
